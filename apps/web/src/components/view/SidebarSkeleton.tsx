...
export function SidebarSkeleton() {
  ...
  // On mobile, the sidebar renders as a Sheet, so no skeleton needed
  if (isMobile) {
    return null;
  }
  ...
  if (isCollapsed) {
    return (
      <aside
        className={cn(
          "w-16 shrink-0 flex flex-col bg-secondary overflow-hidden",
      ...
      >
        <div className="w-16">
          ...
          {/* Navigation skeleton - icons only */}
          <nav className="flex-1 px-2 py-4 space-y-1">
            <div className="h-10 w-full rounded-md bg-border/40 animate-pulse" />
            <div className="h-10 w-full rounded-md bg-border/40 animate-pulse" />
          ...
          {/* User Profile skeleton - avatar only */}
  ...
  return (
    <aside
      className={cn(
        "w-[260px] shrink-0 flex flex-col bg-secondary overflow-hidden",
        "transition-[width] duration-200 ease-linear"
      )}
    >
