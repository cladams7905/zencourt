...
import type { VideoPreviewTimelineLayoutItem } from "@web/src/components/listings/content/video/domain/videoPreviewTimelineViewModel";
...
export function VideoPreviewTimelineRuler({
  ...
  for (let tickIndex = 0; tickIndex <= tickCount; tickIndex += 1) {
    ...
    const leftPx =
      item.startPx + Math.max(0, Math.min(1, ratio)) * item.widthPx;
    ...
    ticks.push(
      ...
      >
        <div
          className={
            isSecondMark ? "h-4 w-px bg-border" : "h-2.5 w-px bg-border/70"
          }
        />
        {isSecondMark ? (
