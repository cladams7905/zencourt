...
function getVideoPreviewSegmentKey(
  segment: TimelinePreviewResolvedSegment
): string {
      ...
      return [
        ...
        labels.get(key) ??
          formatVideoPreviewClipLabel(
            segment.category?.trim() || segment.clipId
          )
      ];
    })
  );
...
export function buildVideoPreviewTimelineLayout(params: {
  ...
  return {
    items,
    contentWidthPx: items.length === 0 ? 0 : items[items.length - 1]!.endPx
  };
}
