...
describe("VideoPreviewModal", () => {
  beforeEach(() => {
    ...
    URL.createObjectURL =
      mockCreateObjectURL.mockReturnValue("blob:reel-preview");
  ...
  it("renders a player, timeline items, and editable hook/caption fields", () => {
    ...
    expect(
      screen.getAllByAltText("Exterior clip thumbnail").length
    ).toBeGreaterThan(screen.getAllByAltText("Kitchen clip thumbnail").length);
  ...
  it("updates only the targeted draft field during regeneration", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "Regenerate header" }));
    await user.click(
  ...
  it("resets overlay control values before closing on discard confirm", async () => {
    ...
    render(
      <StatefulVideoPreviewModal
        preview={createSelectedPreviewWithAddressOverlay()}
      />
    );
  ...
  it("opens the discard confirmation for dialog dismiss requests when dirty", async () => {
    ...
    await user.type(screen.getByLabelText("Header"), "Updated hook");
    await user.click(
      screen.getByRole("button", { name: "Mock dialog dismiss" })
    );
  ...
  it("shows user media in the add-clip popover and appends it to the timeline", async () => {
    ...
    mockSeekTo.mockClear();
    await user.click(
      screen.getByRole("button", { name: /Uploaded walkthrough/i })
    );
    ...
    fireEvent.click(screen.getByTestId("timeline-delete-user-media:media-1-2"));
