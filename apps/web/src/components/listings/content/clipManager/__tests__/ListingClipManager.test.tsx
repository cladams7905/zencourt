...
describe("ListingClipManager", () => {
  ...
  it("links the summary card to the child clips route and preserves query state", () => {
    ...
    expect(link).toHaveAttribute(
      "href",
      "/listings/listing-1/content/clips?mediaType=videos&filter=property_features&page=2"
    );
  ...
  it("renders the selected clip details inline on mobile instead of the desktop detail pane", () => {
    ...
    expect(screen.getByTestId("mobile-clip-detail-clip-1")).toHaveTextContent(
      "Version"
    );
  });
  ...
  it("falls back to the last successful thumbnail and duration after a regeneration fails", () => {
    ...
    expect(screen.getByAltText("Kitchen")).toHaveAttribute(
      "src",
      "https://thumb"
    );
  ...
  it("falls back to the last successful version in the UI after canceling regeneration", async () => {
    ...
    expect(screen.queryByText("Regenerating now")).not.toBeInTheDocument();
