...
import type { ListingCreateMediaTab } from "@web/src/components/listings/content/shared/constants";
...
>(previewItems: T[], captionItems: { id: string }[]): T[] {
...
function applyResetReason(
  ...
  setters: {
    setPreviewItems: React.Dispatch<
      React.SetStateAction<ListingImagePreviewItem[]>
    >;
...
function handleTemplateRenderRequestError(params: {
  ...
  requestRef: React.MutableRefObject<number>;
  setRenderError: React.Dispatch<React.SetStateAction<string | null>>;
  setIsTemplateRenderingUnavailable: React.Dispatch<
...
export function useTemplateRender(
  ...
  const streamedPreviews = React.useMemo(() => {
    ...
    return filterPreviewItemsForActiveCaptionItems(
      previews,
      templateCaptionItems
    );
