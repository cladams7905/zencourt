...
const mockBuildOrshotCaptionItems = jest.fn();
const mockMapOrshotItemsToPreviewItems = jest.fn();

jest.mock("@web/src/components/listings/create/domain/listingCreateUtils", () => ({
  buildOrshotCaptionItems: (...args: unknown[]) => mockBuildOrshotCaptionItems(...args),
  mapOrshotItemsToPreviewItems: (...args: unknown[]) =>
    mockMapOrshotItemsToPreviewItems(...args)
}));

describe("useOrshotRender", () => {
  ...
  it("does not fetch when caption items sanitize to empty", () => {
    mockBuildOrshotCaptionItems.mockReturnValue([]);
  ...
  it("maps successful render result", async () => {
    mockBuildOrshotCaptionItems.mockReturnValue([{ id: "cap-1", hook: "h", caption: null, body: [] }]);
    mockMapOrshotItemsToPreviewItems.mockReturnValue([
      {
        id: "preview-1",
        variationNumber: 1,
        hook: "h",
  ...
  it("disables orshot after ORSHOT_API_KEY error", async () => {
    mockBuildOrshotCaptionItems.mockReturnValue([{ id: "cap-1", hook: "h", caption: null, body: [] }]);
    (global.fetch as jest.Mock).mockResolvedValue({
      ok: false,
      json: async () => ({ message: "missing ORSHOT_API_KEY" })
    });
    ...
    await waitFor(() => {
      expect(result.current.renderError).toBe("Orshot is unavailable. Showing fallback.");
    });
  });
