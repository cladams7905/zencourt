...
import { usePlanProcessingFlow } from "@web/src/components/listings/stage/processing/domain/hooks/usePlanProcessingFlow";

describe("usePlanProcessingFlow", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it("restores batch ids from sessionStorage and navigates when all batch images are terminal", async () => {
    ...
    await waitFor(() => {
      expect(navigate).toHaveBeenCalledWith("/listings/l1/stage/plan");
    });
    expect(mockEmitListingSidebarUpdate).not.toHaveBeenCalled();
    expect(window.sessionStorage.getItem("listing-plan-processing:l1")).toBeNull();
  });
});
