...
export async function createVideoGenBatch(video: InsertDBVideoGenBatch): Promise<void> {
  return withDbErrorHandling(
    ...
    },
    {
      ...
      errorMessage: "Failed to create video generation batch. Please try again."
    }
  );
}
...
export async function updateVideoGenBatch(
  ...
  return withDbErrorHandling(
    ...
    },
    {
      ...
      context: { videoId },
      errorMessage: "Failed to update video generation batch. Please try again."
    }
  );
}
...
export async function createVideoGenJobsBatch(jobs: InsertDBVideoGenJob[]): Promise<void> {
  if (jobs.length === 0) {
    return;
  }
  ...
  return withDbErrorHandling(
    ...
    },
    {
      ...
      errorMessage: "Failed to create video generation jobs. Please try again."
    }
  );
}
