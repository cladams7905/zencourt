...
import {
  ...
  updateUserProfile,
  updateWritingStyle
} from "@web/src/server/models/user/additional/mutations";
...
describe("userAdditional mutations", () => {
  ...
  it("completes welcome survey", async () => {
    ...
    await expect(
      completeWelcomeSurvey("u1", { location: "X" } as never)
    ).resolves.toEqual({
      userId: "u1"
    });
  });
  ...
  it("updates user profile and location", async () => {
    mockInsertReturning.mockResolvedValueOnce([
      { userId: "u1", agentName: "Agent" }
    ]);
    await expect(
      updateUserProfile("u1", { agentName: "Agent" })
    ).resolves.toEqual({
      userId: "u1",
      agentName: "Agent"
    });
    ...
    mockInsertReturning.mockResolvedValueOnce([
      { userId: "u1", location: "CA" }
    ]);
    await expect(
      updateUserLocation("u1", "CA" as never, {
        county: "Orange",
        serviceAreas: ["Irvine"]
      })
  ...
  it("updates writing style", async () => {
    ...
    await expect(
      updateWritingStyle("u1", { writingToneLevel: "warm" } as never)
    ).resolves.toEqual({ userId: "u1", writingToneLevel: "warm" });
  });

  it("marks profile and writing style completion", async () => {
    mockUpdateReturning.mockResolvedValueOnce([
      { userId: "u1", profileCompletedAt: new Date() }
    ]);
