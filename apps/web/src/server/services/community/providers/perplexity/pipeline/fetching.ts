...
import { getCategoryDisplayLimit, shouldIncludeServiceAreasInCache, type AudienceSegment, type CategoryKey, type CommunityCategoryPayload } from "@web/src/server/services/community/config";
import { getUtcMonthKey } from "../../../shared/common";
import { buildCommunityCategoryPayload } from "./parsing";
import { buildPerplexityCommunityMessages, getAudienceLabel } from "../transport/prompts";
import { buildPerplexityResponseFormat } from "../transport/schema";
import { getCachedPerplexityCategoryPayload, getCachedPerplexityMonthlyEventsPayload, setCachedPerplexityCategoryPayload, setCachedPerplexityMonthlyEventsPayload } from "../cache";
...
export async function fetchPerplexityCategoryPayload(params: {
  ...
  if (!params.forceRefresh) {
    ...
    if (cached) {
      logger.info({ zipCode, category, audience, cached: true }, "Perplexity category cache hit");
      return cached;
    }
  }
  ...
  const messages = buildPerplexityCommunityMessages({
    ...
    limit,
    extraInstructions: [params.extraInstructions, avoidInstructions].filter(Boolean).join(" ")
  });
  ...
  if (!response) {
    logger.warn({ zipCode, category, audience }, "Perplexity category request failed");
    return null;
  }
  ...
  if (!payload) {
    logger.warn({ zipCode, category, audience }, "Perplexity category payload empty");
    return null;
  }
...
export async function fetchPerplexityMonthlyEventsPayload(params: {
  ...
  if (cached) {
    logger.info({ zipCode: params.zipCode, monthKey, audience: params.audience }, "Perplexity monthly events cache hit");
    return cached;
  }
  ...
  if (!response) {
    logger.warn({ zipCode: params.zipCode, monthKey, audience: params.audience }, "Perplexity monthly events request failed");
    return null;
  }
  ...
  if (!payload) {
    logger.warn({ zipCode: params.zipCode, monthKey, audience: params.audience }, "Perplexity monthly events payload empty");
    return null;
  }
