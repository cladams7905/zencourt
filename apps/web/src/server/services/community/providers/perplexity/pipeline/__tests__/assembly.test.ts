...
} from "@web/src/server/services/community/providers/perplexity/pipeline/assembly";
...
describe("perplexity assembly", () => {
  it("returns none found when payload is empty", () => {
    ...
    expect(buildCategoryList("dining", { items: [] } as never)).toBe("- (none found)");
  });

  it("formats category list when items are present", () => {
    expect(
      buildCategoryList("dining", { items: [{ name: "Cafe" }] } as never, "families")
    ).toBe("- Cafe");
  });
