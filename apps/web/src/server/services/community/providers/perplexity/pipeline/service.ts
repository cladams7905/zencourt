...
export async function getPerplexityMonthlyEventsSection(params: {
  ...
  const value = formatPerplexityCategoryList("community_events", monthlyPayload.items, audienceLabel);
...
export async function getPerplexityCommunityData(params: {
  ...
  if (Object.keys(seasonalSections).length > 0) {
    logger.info({ zipCode, audience, monthKey }, "Perplexity monthly events cached");
  }
