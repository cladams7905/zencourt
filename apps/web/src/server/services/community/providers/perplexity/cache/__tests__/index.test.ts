...
} from "@web/src/server/services/community/providers/perplexity/cache";

describe("perplexity cache", () => {
  const configMock = jest.requireMock("@web/src/server/services/community/config");
  const commonMock = jest.requireMock("@web/src/server/services/community/shared/common");
  const redisMod = jest.requireMock("@web/src/server/services/community/shared/redis");
  ...
  it("reads and writes category payloads", async () => {
    ...
    await expect(
      getCachedPerplexityCategoryPayload({ zipCode: "78701", category: "dining" as never })
  ...
  it("returns null/noop when redis client is unavailable", async () => {
    const createRedisClientGetter = redisMod.createRedisClientGetter as jest.Mock;
    createRedisClientGetter.mockImplementationOnce(() => () => null);
    jest.resetModules();
    const mod = await import("@web/src/server/services/community/providers/perplexity/cache");

    await expect(
      mod.getCachedPerplexityCategoryPayload({ zipCode: "78701", category: "dining" as never })
    ).resolves.toBeNull();
    await expect(
  ...
  it("builds monthly cache key and reads/writes payload", async () => {
    ...
    await expect(
      getCachedPerplexityMonthlyEventsPayload({ zipCode: "78701", monthKey: "february" })
