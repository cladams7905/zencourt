...
jest.mock("@web/src/server/services/community/providers/perplexity/pipeline/service", () => ({
  getPerplexityCommunityDataForCategories: (...args: unknown[]) =>
    mockGetByCategories(...args),
  getPerplexityMonthlyEventsSection: (...args: unknown[]) => mockGetMonthlyEvents(...args),
  prefetchPerplexityCategories: (...args: unknown[]) => mockPrefetchCategories(...args)
}));
...
} from "@web/src/server/services/community/providers/perplexity/pipeline/ops";
...
describe("perplexity ops", () => {
  beforeEach(() => {
    ...
    mockToOriginLocationInput.mockReturnValue({ city: "Austin", state: "TX", lat: 1, lng: 2 });
  });

  it("returns null for missing zip", async () => {
    await expect(
      getPerplexityCommunityDataByZipAndAudienceForCategories("", ["dining"] as never)
    ).resolves.toBeNull();
  ...
  it("returns null when location cannot be resolved", async () => {
    ...
    await expect(
      getPerplexityCommunityDataByZipAndAudienceForCategories("78701", ["dining"] as never)
    ).resolves.toBeNull();
  });
  ...
  it("routes category fetch through resolved location", async () => {
    ...
    const result = await getPerplexityCommunityDataByZipAndAudienceForCategories(
      "78701",
      ["dining"] as never,
      "families"
    );
  ...
  it("routes monthly events through resolved location", async () => {
    ...
    await expect(getPerplexityMonthlyEventsSectionByZip("78701", "families")).resolves.toEqual({
      key: "k",
      value: "v"
    });
  ...
  it("prefetches categories when zip and location are valid", async () => {
    await prefetchPerplexityCategoriesByZip("78701", ["dining"] as never, "families");
    expect(mockPrefetchCategories).toHaveBeenCalled();
  });

  it("noops prefetch for missing zip", async () => {
    await prefetchPerplexityCategoriesByZip("", ["dining"] as never, "families");
