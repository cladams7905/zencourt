import { formatPerplexityCategoryList } from "@web/src/server/services/community/providers/perplexity/pipeline/formatting";
...
describe("perplexity formatting", () => {
  ...
  it("formats dining items with cuisine and audience label", () => {
    ...
    expect(result).toContain("Why suitable for growing families: Family-friendly");
  });

  it("formats category-specific fields for events and nature", () => {
    const events = formatPerplexityCategoryList(
      "community_events",
      [{ name: "Farmers Market", dates: "Saturdays" }] as never
    );
    const nature = formatPerplexityCategoryList(
      "nature_outdoors",
      [{ name: "Lake Trail", disclaimer: "Weather dependent" }] as never
    );
