import {
  ...
  getAudienceLabel
} from "@web/src/server/services/community/providers/perplexity/transport/prompts";
...
describe("perplexity prompts", () => {
  ...
  it("builds prompts with service areas and affordability notes", () => {
    ...
    expect(messages[1].content).toContain("Service areas to prioritize: Austin, TX.");
    expect(messages[1].content).toContain("Prioritize affordable, budget-friendly options.");
