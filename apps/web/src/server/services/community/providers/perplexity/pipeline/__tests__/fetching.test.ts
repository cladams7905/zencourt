...
jest.mock("@web/src/server/services/community/providers/perplexity/pipeline/parsing", () => ({
  buildCommunityCategoryPayload: (...args: unknown[]) =>
    mockBuildCommunityCategoryPayload(...args)
}));

jest.mock("@web/src/server/services/community/providers/perplexity/transport/prompts", () => ({
  buildPerplexityCommunityMessages: (...args: unknown[]) => mockBuildMessages(...args),
  getAudienceLabel: jest.fn(() => "local residents")
}));

jest.mock("@web/src/server/services/community/providers/perplexity/transport/schema", () => ({
  buildPerplexityResponseFormat: (...args: unknown[]) => mockBuildResponseFormat(...args)
}));

jest.mock("@web/src/server/services/community/providers/perplexity/cache", () => ({
  getCachedPerplexityCategoryPayload: (...args: unknown[]) => mockGetCachedCategory(...args),
  getCachedPerplexityMonthlyEventsPayload: (...args: unknown[]) => mockGetCachedMonthly(...args),
  setCachedPerplexityCategoryPayload: (...args: unknown[]) => mockSetCachedCategory(...args),
  setCachedPerplexityMonthlyEventsPayload: (...args: unknown[]) => mockSetCachedMonthly(...args)
}));

jest.mock("@web/src/server/services/community/providers/perplexity/transport/client", () => ({
...
import {
  ...
  formatAudienceLabel
} from "@web/src/server/services/community/providers/perplexity/pipeline/fetching";
...
describe("perplexity fetching", () => {
  beforeEach(() => {
    ...
    mockBuildCommunityCategoryPayload.mockReturnValue({ items: [{ name: "Cafe" }] });
  });

  it("returns cached category payload when available", async () => {
    mockGetCachedCategory.mockResolvedValueOnce({ items: [{ name: "Cached" }] });
  ...
  it("fetches and caches category payload when cache misses", async () => {
    ...
    mockRequestPerplexity.mockResolvedValueOnce({ choices: [{ message: { content: "{}" } }] });
