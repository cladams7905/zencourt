...
jest.mock("@web/src/server/services/community/providers/perplexity/pipeline/assembly", () => ({
  buildCategoryList: (...args: unknown[]) => mockBuildCategoryList(...args),
  buildPerplexityCommunityData: (...args: unknown[]) => mockBuildCommunityData(...args)
}));

jest.mock("@web/src/server/services/community/providers/perplexity/pipeline/fetching", () => ({
  fetchPerplexityCategoryPayload: (...args: unknown[]) => mockFetchCategoryPayload(...args),
  fetchPerplexityMonthlyEventsPayload: (...args: unknown[]) => mockFetchMonthlyPayload(...args),
  formatAudienceLabel: (...args: unknown[]) => mockFormatAudienceLabel(...args)
}));

jest.mock("@web/src/server/services/community/providers/perplexity/pipeline/formatting", () => ({
  formatPerplexityCategoryList: (...args: unknown[]) => mockFormatPerplexityCategoryList(...args)
}));
...
} from "@web/src/server/services/community/providers/perplexity/pipeline/service";
...
describe("perplexity service", () => {
  ...
  it("returns keyed monthly events section", async () => {
    mockFetchMonthlyPayload.mockResolvedValueOnce({ items: [{ name: "Event" }] });
