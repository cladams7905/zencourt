...
jest.mock("@web/src/server/services/community/providers/google/transport/client", () => ({
...
jest.mock("@web/src/server/services/community/providers/google/core/geo", () => ({
  DistanceCache: class {
...
import {
  ...
  communityCache
} from "@web/src/server/services/community/providers/google/pipeline/shared";

describe("google pipeline shared", () => {
  const geoMock = jest.requireMock("@web/src/server/services/community/providers/google/core/geo");
  ...
  beforeEach(() => {
    ...
    (communityCache.setCachedPlaceDetails as jest.Mock).mockResolvedValue(undefined);
  });

  it("returns query overrides for education libraries", () => {
    expect(getQueryOverrides("education", "best library")).toEqual({ minReviews: 10 });
  ...
  it("reads and caches place details", async () => {
    ...
    expect(communityCache.setCachedPlaceDetails).toHaveBeenCalledWith("p1", { id: "p1" });
  });

  it("builds geo runtime context", async () => {
    geoMock.loadCityDataset.mockResolvedValueOnce([{ city: "Austin", state_id: "TX" }]);
    geoMock.resolveServiceAreaCenters.mockReturnValueOnce([{ city: "Austin" }]);

    const result = await buildGeoRuntimeContext(
      "78701",
      { city: "Austin", state_id: "TX", lat: 1, lng: 2 },
      ["Austin,TX"]
    );
