...
jest.mock("@web/src/server/services/community/shared/cityDescription", () => ({
  fetchCityDescription: (...args: unknown[]) => mockFetchCityDescription(...args)
}));

jest.mock("@web/src/server/services/community/providers/google/core/audience", () => ({
  buildAudienceAugmentDelta: (...args: unknown[]) => mockBuildAudienceAugmentDelta(...args)
}));

jest.mock("@web/src/server/services/community/providers/google/core/communityLists", () => ({
  applyAudienceDelta: (...args: unknown[]) => mockApplyAudienceDelta(...args),
  getAudienceSkipCategories: (...args: unknown[]) => mockGetAudienceSkipCategories(...args),
  trimCommunityDataLists: (...args: unknown[]) => mockTrimCommunityDataLists(...args)
}));
...
jest.mock("@web/src/server/services/community/providers/google/pipeline/service", () => ({
...
jest.mock("@web/src/server/services/community/providers/google/pipeline/shared", () => ({
  ...
  buildGeoRuntimeContext: (...args: unknown[]) => mockBuildGeoRuntimeContext(...args),
  communityCache: {
    getCachedAudienceDelta: (...args: unknown[]) => mockGetCachedAudienceDelta(...args),
    setCachedAudienceDelta: (...args: unknown[]) => mockSetCachedAudienceDelta(...args),
    getCachedCityDescription: (...args: unknown[]) => mockGetCachedCityDescription(...args),
    setCachedCityDescription: (...args: unknown[]) => mockSetCachedCityDescription(...args)
  },
...
import {
  ...
  getCityDescription
} from "@web/src/server/services/community/providers/google/pipeline/audience";
...
describe("google audience pipeline", () => {
  beforeEach(() => {
    ...
    mockResolveLocationOrWarn.mockResolvedValue({ city: "Austin", state_id: "TX", lat: 1, lng: 2 });
    mockBuildGeoRuntimeContext.mockResolvedValue({ distanceCache: {}, serviceAreaCache: null });
  ...
  it("returns null when base community data is unavailable", async () => {
    ...
    await expect(getCommunityDataByZipAndAudience("78701", "families")).resolves.toBeNull();
  });
  ...
  it("reads and caches city descriptions", async () => {
    ...
    mockFetchCityDescription.mockResolvedValueOnce({ description: "Nice city" });
