...
jest.mock("@web/src/server/services/community/providers/google/core/search", () => ({
  fetchScoredPlacesForQueries: (...args: unknown[]) => mockFetchScoredPlacesForQueries(...args),
  getSearchAnchors: (...args: unknown[]) => mockGetSearchAnchors(...args)
}));

jest.mock("@web/src/server/services/community/providers/google/core/places/index", () => ({
  formatPlaceList: (...args: unknown[]) => mockFormatPlaceList(...args),
  dedupePlaces: (...args: unknown[]) => mockDedupePlaces(...args),
  hydratePlacesFromItems: (...args: unknown[]) => mockHydratePlacesFromItems(...args)
}));

jest.mock("@web/src/server/services/community/providers/google/core/pools", () => ({
  getPooledCategoryPlaces: (...args: unknown[]) => mockGetPooledCategoryPlaces(...args)
}));

jest.mock("@web/src/server/services/community/providers/google/core/seasonal", () => ({
  buildSeasonalQueries: (...args: unknown[]) => mockBuildSeasonalQueries(...args),
  ...
  pickSeasonalCategories: (...args: unknown[]) => mockPickSeasonalCategories(...args)
}));

jest.mock("@web/src/server/services/community/config", () => ({
  AUDIENCE_AUGMENT_CATEGORIES: ["dining", "education"],
  getAllAudienceAugmentQueries: (...args: unknown[]) => mockGetAllAudienceAugmentQueries(...args),
...
jest.mock("@web/src/server/services/community/shared/apiCost", () => ({
  estimateGoogleCallsCostUsd: (...args: unknown[]) => mockEstimateGoogleCallsCostUsd(...args)
}));

import { buildAudienceAugmentDelta } from "@web/src/server/services/community/providers/google/core/audience";
...
describe("google audience core", () => {
  ...
  beforeEach(() => {
    ...
    mockMergeUniqueQueries.mockImplementation((a: string[], b: string[]) => [...a, ...b]);
    ...
    mockFetchScoredPlacesForQueries.mockResolvedValue([{ name: "Cafe", placeId: "p1" }]);
    mockGetPooledCategoryPlaces.mockImplementation(
      async (_cache: unknown, _logger: unknown, _ctx: unknown, fetchFn: () => Promise<unknown>) => {
        await fetchFn();
        return [{ placeId: "p1" }];
      }
    );
