...
jest.mock("@web/src/server/services/community/providers/google/core/search", () => ({
  ...
  fetchPlacesWithAnchors: (...args: unknown[]) => mockFetchPlacesWithAnchors(...args),
...
jest.mock("@web/src/server/services/community/providers/google/core/places/details", () => ({
...
import { fetchGroupedPlaces } from "@web/src/server/services/community/providers/google/pipeline/workflows/fetching";
...
describe("google fetching workflow", () => {
  ...
  it("fetches grouped category and neighborhood places", async () => {
    const result = await fetchGroupedPlaces({
      zipCode: "78701",
      location: { city: "Austin", state_id: "TX", lat: 1, lng: 2 },
      distanceCache: {} as never,
      serviceAreaCache: null,
      categoryQueries: [
        { key: "dining", queries: ["q1"], seasonalQueries: new Set(), max: 5 }
      ],
      neighborhoodsToFetch: [{ key: "neighborhoods_general", query: "q", max: 3 }] as never
    });

    expect(result.grouped.dining).toEqual([{ name: "Cafe" }]);
    expect(result.grouped.neighborhoods_general).toEqual([{ name: "Neighborhood" }]);
