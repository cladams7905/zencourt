import {
  ...
  fetchPlaces,
  fetchPlacesNearby
} from "@web/src/server/services/community/providers/google/transport/client";
...
describe("google transport client", () => {
  ...
  it("returns empty/null when api key missing", async () => {
    delete process.env.GOOGLE_API_KEY;
    await expect(fetchPlaces("coffee", { lat: 1, lng: 2 }, 5, 1000)).resolves.toEqual([]);
    await expect(fetchPlaceDetails("pid")).resolves.toBeNull();
  });

  it("fetches places and nearby places", async () => {
    (global.fetch as jest.Mock)
      .mockResolvedValueOnce({ ok: true, json: async () => ({ places: [{ id: "p1" }] }) })
      .mockResolvedValueOnce({ ok: true, json: async () => ({ places: [{ id: "p2" }] }) });

    await expect(fetchPlaces("coffee", { lat: 1, lng: 2 }, 5, 1000)).resolves.toEqual([
      { id: "p1" }
    ]);
    await expect(fetchPlacesNearby(["restaurant"], { lat: 1, lng: 2 }, 5, 1000)).resolves.toEqual([
      { id: "p2" }
    ]);
  });
