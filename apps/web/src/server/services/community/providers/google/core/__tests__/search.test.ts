...
} from "@web/src/server/services/community/providers/google/core/search";
...
describe("google search", () => {
  ...
  it("dedupes search anchors and falls back to origin", () => {
    const anchors = getSearchAnchors(
  ...
  it("fetches places with single anchor for low-priority category", async () => {
    ...
    await fetchPlacesWithAnchors(
      ...
      { lat: 1, lng: 2 } as never,
      4,
      [{ lat: 0.1, lng: 0.1 }, { lat: 0.2, lng: 0.2 }],
      "education" as never
    );
  ...
  it("filters and scores places", () => {
    const distanceCache = { getDistanceKm: jest.fn().mockReturnValue(2) } as never;
  ...
  it("applies neighborhood reject terms and service-area distance", () => {
    const distanceCache = { getDistanceKm: jest.fn().mockReturnValue(2) } as never;
    const serviceAreaCache = { getDistanceKm: jest.fn().mockReturnValue(3) } as never;
