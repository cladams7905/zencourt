...
jest.mock("@web/src/server/services/community/providers/google/core/base", () => ({
  buildBaseCategoryFieldValues: (...args: unknown[]) =>
    mockBuildBaseCategoryFieldValues(...args),
  buildCategoryQueryPlan: (...args: unknown[]) => mockBuildCategoryQueryPlan(...args),
...
jest.mock("@web/src/server/services/community/providers/google/pipeline/shared", () => ({
  ...
  communityCache: {
    getCachedCommunityData: (...args: unknown[]) => mockGetCachedCommunityData(...args),
    setCachedCommunityData: (...args: unknown[]) => mockSetCachedCommunityData(...args)
  },
  logger: { info: (...args: unknown[]) => mockLoggerInfo(...args) }
}));

jest.mock("@web/src/server/services/community/providers/google/pipeline/workflows/planning", () => ({
  loadBaseCachePlan: (...args: unknown[]) => mockLoadBaseCachePlan(...args)
}));

jest.mock("@web/src/server/services/community/providers/google/pipeline/workflows/fetching", () => ({
  fetchGroupedPlaces: (...args: unknown[]) => mockFetchGroupedPlaces(...args)
}));

jest.mock("@web/src/server/services/community/providers/google/pipeline/workflows/assembly", () => ({
...
import { getCommunityDataByZip } from "@web/src/server/services/community/providers/google/pipeline/service";
...
describe("google pipeline service", () => {
  beforeEach(() => {
    ...
    mockBuildAndPersistCategoryListMap.mockResolvedValue(new Map([["dining_list", "- D1"]]));
