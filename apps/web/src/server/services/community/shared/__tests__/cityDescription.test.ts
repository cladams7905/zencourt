...
jest.mock("@web/src/server/services/community/providers/perplexity/pipeline/cityDescription", () => ({
  fetchPerplexityCityDescription: (...args: unknown[]) =>
    mockFetchPerplexityCityDescription(...args)
}));

import { fetchCityDescription } from "@web/src/server/services/community/shared/cityDescription";
...
describe("shared cityDescription", () => {
  ...
  it("delegates to perplexity provider when selected", async () => {
    ...
    mockFetchPerplexityCityDescription.mockResolvedValueOnce({ description: "A great city" });
  ...
  it("returns null when anthropic key is missing", async () => {
    ...
    await expect(fetchCityDescription("Austin", "TX", logger)).resolves.toBeNull();
