async function importGeoWithMocks(params: {
  ...
  const mod =
    await import("@web/src/server/services/communityData/providers/google/core/geo");
...
describe("google geo", () => {
  ...
  it("returns empty dataset for empty csv file", async () => {
    const { mod } = await importGeoWithMocks({
  ...
  it("parses csv and resolves zip locations", async () => {
    ...
    const byPreferredCity = await mod.resolveZipLocation(
      "78701",
      "Austin",
      "TX",
      logger
  ...
  it("skips rows with invalid coordinates and returns null for missing zip", async () => {
    ...
    await expect(mod.loadCityDataset(logger)).resolves.toEqual([]);
    await expect(
      mod.resolveZipLocation("73301", null, null, logger)
    ).resolves.toBeNull();
  });
  ...
  it("returns null service centers for empty or unknown service areas", async () => {
    ...
    expect(
      mod.resolveServiceAreaCenters([], { state_id: "TX" } as never, records)
    ).toBeNull();
    expect(
      mod.resolveServiceAreaCenters(
        ["Unknown City"],
        { state_id: "TX" } as never,
        records
      )
