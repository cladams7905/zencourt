...
jest.mock("@web/src/server/actions/contentGeneration/domain/marketLocation", () => ({
...
import { resolveContentContext } from "@web/src/server/actions/contentGeneration/context";
import { DomainValidationError, DomainDependencyError } from "@web/src/server/errors/domain";
  ...
  describe("resolveContentContext", () => {
    ...
    it("fetches community context when category is community and location is valid", async () => {
      ...
      const result = await resolveContentContext({
        ...
        snapshot: { ...snapshot, location: "Austin, TX 78701", serviceAreas: [] }
      });
