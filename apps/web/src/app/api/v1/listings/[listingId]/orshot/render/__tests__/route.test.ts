...
describe("listing orshot render route", () => {
  async function loadRoute() {
    ...
    const mockRenderListingOrshotBatch = jest.fn().mockResolvedValue({
      items: [
        {
          templateId: "tpl-1",
  ...
  it("returns empty result when caption items sanitize to empty", async () => {
    const { POST, mockRenderListingOrshotBatch } = await loadRoute();
    const request = {
      json: async () => ({
    ...
    expect(mockRenderListingOrshotBatch).not.toHaveBeenCalled();
  });
  ...
  it("sanitizes caption items and returns render result", async () => {
    ...
    expect(response.status).toBe(200);
    expect(mockRenderListingOrshotBatch).toHaveBeenCalledWith(
